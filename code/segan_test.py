...
class SEGAN(object):
    ...
    def build_model(self):
        ...
        d_optimizer = tf.train.RMSPropOptimizer(learning_rate=self.d_learning_rate)
        g_optimizer = tf.train.RMSPropOptimizer(learning_rate=self.g_learning_rate)

        d_grads = d_optimizer.compute_gradients(self.d_loss,
                                                dis_variables)  # Only update the weights for the discriminator network.
        g_grads = g_optimizer.compute_gradients(self.g_loss,
                                                gen_variables)  # Only update the weights for the generator network.

        self.update_D = d_optimizer.apply_gradients(d_grads)
        self.update_G = g_optimizer.apply_gradients(g_grads)
    ...
    def train(self):
        ...
        for epoch in range(0, self.epoch):
            ...
            for idx in range(0, self.data_len-1):
                # update D network
                _, summary_str, d_loss = self.sess.run([self.update_D, self.d_loss_sum, self.d_loss])
                self.writer.add_summary(summary_str, counter)

                # update G network
                _, summary_str, g_loss = self.sess.run([self.update_G, self.g_loss_sum, self.g_loss])
                self.writer.add_summary(summary_str, counter)
