...
class SEGAN(object):
    ...
    def build_model(self):
        ...
        self.d_rl_loss = tf.reduce_mean(tf.squared_difference(d_rl_logits, 1.))

        self.d_fk_loss = tf.reduce_mean(tf.squared_difference(d_fk_logits, 0.))
        self.g_adv_loss = tf.reduce_mean(tf.squared_difference(d_fk_logits, 1.))
        self.d_loss = self.d_rl_loss + self.d_fk_loss

        self.g_l1_loss = self.l1_lambda * tf.reduce_mean(tf.abs(tf.subtract(G, self.clean_wav)))
        self.g_loss = self.g_adv_loss + self.g_l1_loss
        ...
        self.d_rl_loss_sum = tf.summary.scalar("d_rl_loss", self.d_rl_loss)
        self.d_fk_loss_sum = tf.summary.scalar("d_fk_loss", self.d_fk_loss)
        self.d_loss_sum = tf.summary.scalar("d_loss", self.d_loss)

        self.g_loss_l1_sum = tf.summary.scalar("g_l1_loss", self.g_l1_loss)
        self.g_loss_adv_sum = tf.summary.scalar("g_adv_loss", self.g_adv_loss)
